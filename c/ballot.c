...
// Types
typedef struct addr_t { uint8_t addr[ADDRESS_SIZE]; } addr_t;
typedef struct topic_t { char str[MAX_TOPIC_LEN]; } topic_t;

// Voter
// First u8: 1 means has voter right
// Second u8: 1 means yea, 2 means nay
typedef struct voter_t { uint8_t state[2]; } voter_t;
...
int main(int argc, char *argv[]) {
  ...
  // Load topic
  pvm_load(topic_key, strlen(TOPIC_KEY), (uint8_t *)topic.str, MAX_TOPIC_LEN, NULL);
